            ...
            <el-form-item label="订单状态">
                    <div class="base">状态:
                                            ...
                                            <el-button type="warning" size="small"
                                                v-else-if="orderData.orderStatus == 4">取消订单</el-button>
...
onMounted(() => {
    ...
    if ("orderUUID" in queryDict && "objectId" in queryDict) {
      ...
      link("/Order/info/orderUUID", 'GET',{}, {orderUUID: orderUUID.value, objectId: objectId.value}, {}).then(response => {
...
const orderStatusEvent = function (num) { 
    ...
    //
    //状态0:待发货
    //状态1:已发货  填写快递单号自动修改状态
    //状态2:已收货  允许退货
    //状态3:申请退货    同意退货(返回取消订单)/拒绝退货(返回已收货)
    //状态4:取消订单
    //
